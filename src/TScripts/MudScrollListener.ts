...
class MudScrollListener {
    ...
    listenForScroll(dotnetReference:any, selector:any) {
        //if selector is null, attach to document
        let element = selector ? document.querySelector(selector) : document;
        // add the event listener
        element.addEventListener('scroll', this.throttleScrollHandler.bind(this, dotnetReference), false);
    }

    // fire the event just once each 100 ms, **it's hardcoded**
    throttleScrollHandler(dotnetReference:any, event:any) {
    ...
    scrollHandler(dotnetReference:any, event:any) {
        try {
