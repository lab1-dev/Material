...
class MudScrollManager {
    ...
    scrollToFragment (elementId:any, behavior:any) {
        let element = document.getElementById(elementId);
        if (element) element.scrollIntoView({behavior, block: 'center', inline: 'start'});
    }

    //scrolls to year in MudDatePicker
    scrollToYear (elementId:any, offset:any) {
        let element = document.getElementById(elementId) as any;
        if (element) element.parentNode.scrollTop = element.offsetTop - element.parentNode.offsetTop - element.scrollHeight * 3;
    }
    ...
    scrollToListItem (elementId:any, increment:any, onEdges:any) {
        let element = document.getElementById(elementId);
        if (element) {
            ...
            //define the height of an disabled component
            let dHeight = eHeight;
            //get the absolute increment
            const absIncrement = Math.abs(increment);
            //check if we are jumping
            if (absIncrement > 1) dHeight = parent.querySelector(".mud-list-component-disabled")?.getBoundingClientRect().height ?? eHeight;
            if (
    ...
    lockScroll (selector:any, lockclass:any) {
