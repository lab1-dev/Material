...
export class MudHelpers{

    public static getTabbableElements(element:any){
    ...
    public static serializeParameter(data:any, spec:any){
        ...
        for (let i in data) {
            ...
            if (typeof currentMember === 'function' || currentMember === null) continue;
            ...
            if (typeof currentMember === 'object') {
                if (Array.isArray(currentMember) || currentMember.length) {
                    ...
                    if (currentMember.length === 0) res[i] = [];
                    else res[i] = this.serializeParameter(currentMember, currentMemberSpec);
                }
            } else {
...
window['getTabbableElements'] = (element) => MudHelpers.getTabbableElements(element);

//from: https://github.com/RemiBou/BrowserInterop
window['serializeParameter'] = (data, spec) => MudHelpers.serializeParameter(data,spec)
