...
declare const window: any;//hack


function registerHelper(){
    window.mudpopoverHelper = {

        calculatePopoverPosition: function (list:any, boundingRect:any, selfRect:any) {
            let top = 0;
            let left = 0;
        ...
        getPositionForFlippedPopver: function (inputArray:any, selector:any, boundingRect:any, selfRect:any) {
            const classList = [];
            for (let i = 0; i < inputArray.length; i++) {
                const component = inputArray[i];
                const replacments = window.mudpopoverHelper.flipClassReplacements[selector][component];
                if (replacments) classList.push(replacments);
                else classList.push(component);
            }
            return window.mudpopoverHelper.calculatePopoverPosition(classList, boundingRect, selfRect);
        },

        placePopover: function (popoverNode:any, classSelector:any) {
            if (popoverNode && popoverNode.parentNode) {
                const id = popoverNode.id.substr(8);
                const popoverContentNode = document.getElementById('popovercontent-' + id) as any;
                if (popoverContentNode&& !popoverContentNode.classList.contains('mud-popover-open')) return;
                if (!popoverContentNode) return;
                if (classSelector) {
                    if (popoverContentNode&& !popoverContentNode.classList.contains(classSelector)) return;
                }
                const boundingRect = popoverNode.parentNode.getBoundingClientRect();
                //console.log('(placePopover)popoverNode:',popoverNode.id,'.BoundingRect:',boundingRect, '.ParentNode:',popoverNode.parentNode.id)
                if (popoverContentNode.classList.contains('mud-popover-relative-width')) popoverContentNode.style.maxWidth = (boundingRect.width) + 'px';
                ...
                //console.log('(placePopover)',postion.top);
                let top = postion.top;
                ...
                if (classList.contains('mud-popover-overflow-flip-onopen') || classList.contains('mud-popover-overflow-flip-always')) {
                    ...
                    let appBarOffset = 0;
                    if (appBarElements.length > 0) appBarOffset = appBarElements[0].getBoundingClientRect().height;
                    ...
                    if (classList.contains('mud-popover-overflow-flip-onopen')) {
                        if (!popoverContentNode.mudPopoverFliped) popoverContentNode.mudPopoverFliped = selector ?? 'none';
                    }
                }
        ...
        placePopoverByNode: function (target:any) {
...
export class MudPopover {

    map:any = {};
    contentObserver:any=null;
    mainContainerClass:any=null;

    constructor() {
        window.mudPopover = this;
        registerHelper();
        //nao tinha abaixo no mud
        //console.log('instanciou mudpopover')
        ...
        //nao tinha acima no mud
    }

    callback(id:any, mutationsList:any, observer:any) {
        for (const mutation of mutationsList) {
            if (mutation.type === 'attributes') {
                const target = mutation.target
                if (target.classList.contains('mud-popover-overflow-flip-onopen') &&
                    !target.classList.contains('mud-popover-open')) {
    ...
    initilize(containerClass:any, flipMargin:any) {
        const mainContent = document.getElementsByClassName(containerClass) as any;
    ...
    connect(id:any) {
        this.initilize(this.mainContainerClass,false);
        ...
        if (popoverNode && popoverNode.parentNode && popoverContentNode) {
            ...
            const resizeObserver = new ResizeObserver(entries => {
                //console.log('(MudPoPover)resizeOberver: entries:',entries.length)
            ...
            resizeObserver.observe(popoverNode.parentNode as any);
