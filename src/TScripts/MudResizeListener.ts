...
class MudResizeListener {
    ...
    listenForResize(dotnetRef:any, options:any) {
        if (this.dotnet) {
    ...
    getBreakpoint(width:any) {
        if (width >= this.options.breakpointDefinitions["Xl"]) return 4;
        else if (width >= this.options.breakpointDefinitions["Lg"]) return 3;
        else if (width >= this.options.breakpointDefinitions["Md"]) return 2;
        else if (width >= this.options.breakpointDefinitions["Sm"]) return 1;
        else return 0;//Xs
    }
}

window['mudResizeListener'] = new MudResizeListener(0);//todo coloquei id como 0
